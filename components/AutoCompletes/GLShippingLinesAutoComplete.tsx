...
import { IShippingLine, IShippingLinesResponse } from '../../interfaces/ShippingLine'

export const shippingLineSchema: SchemaOf<IShippingLine> = yup
  .object({
    ...
    shippingLineTransportTypes: yup.array().notRequired(),
  })
  .defined()
...
interface IGLAutoComplete {
  ...
  placeholder?: string
  onChange: (newValue: IShippingLine | null) => any
  variant?: 'filled' | 'outlined' | 'standard'
  error?: boolean
  value?: IShippingLine
}

export const GLShippingLinesAutoComplete: FC<IGLAutoComplete> = ({
  placeholder,
  onChange,
  label,
  labelIcon,
  ...
  const [data, setData] = useState<IShippingLinesResponse | null>(null)
  ...
  useEffect(() => {
    if (text && text.length > 2) {
      ...
      timeOutRef.current = setTimeout(() => {
        ...
        requests.ShippingLineRequest.getList(tempData)
          .then((res) => {
            setData(res)
          })
  ...
  return (
    <FormControl className={classes.formInput}>
      ...
      <Autocomplete
        ...
        loading={loading}
        onChange={(event: any, newValue: IShippingLine | null) => {
          onChange(newValue)
        }}
        renderInput={(params) => {
          return (
